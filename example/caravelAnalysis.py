...
if __name__ == '__main__':
    ...
    for path in checkable_files:
        ...
        try:
            ...
            sub.subscribe_async(logData.update_acceleration, msg_id=pg.messages.Acceleration)
            sub.subscribe_async(logData.update_wind, msg_id=pg.messages.AbsoluteWind)
            ...
            # Corrent names
            if 'lauv' or 'autonaut' or 'caravel' in key_with_entity_list:

                logData.name = str(key_with_entity_list)
                # Once found the correct vehicle we will load up the Entity List dictionary
                logData.entities = sub._peers[key_with_entity_list]

            if 'lauv' in logData.name or 'autonaut' in logData.name or 'caravel' in logData.name in logData.name:
            ...
            logData.caravel_finish_positions()
            # Create dataframes based on data collected from file
            logData.caravel_create_dataframes()
    ...
    # Remove rejected files from origsinal file list
    ...
    with pd.ExcelWriter("{}.xlsx".format(file_path), engine='xlsxwriter') as writer:
        ...
        'geospatial_vertical_min' : concat_data['DEPTH'].min(),
        'geospatial_vertical_max' : concat_data['DEPTH'].max()
        }
